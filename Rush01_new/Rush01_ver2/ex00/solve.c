...
/*   Updated: 2026/03/08 13:30:10 by ffaria-v         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_solve(char **board, char *clues, int row, int col)
{
	...
	if (row == 4)
	{
		if (ft_check_views(board, clues))
			ft_print(board);
		return (ft_check_views(board, clues));
	}
	i = 0;
	while (++i <= 4)
	{
		...
		{
			board[row][col] = i + '0';
			if (col < 3 && ft_solve(board, clues, row, col + 1))
				return (1);
			if (col == 3 && ft_solve(board, clues, row + 1, 0))
				return (1);
			board[row][col] = '0';
		}
	}
